...
class MyAppState extends State<MyApp> {
  ...
  late final LocationDiagnosticService _locationDiagnosticService =
      LocationDiagnosticService(serverService: _serverService);
  ...
  void _subscribeToLogStream() {
    _logSubscription = _serverService.logStream.listen((log) {
      if (log == 'AUTH_REQUIRED') {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (_) => const UserIdInputScreen()));
      }
    });
  }
  ...
  void _startLocationDiagnosticTimer() {
    _locationDiagnosticTimer?.cancel();
    _locationDiagnosticService.checkAndReport();
    _locationDiagnosticTimer = Timer.periodic(
      const Duration(minutes: 1),
      (_) => _locationDiagnosticService.checkAndReport(),
    );
  }
      ...
      appBar: PreferredSize(
        ...
        child: Container(
          decoration: const BoxDecoration(
              ...
              color: Colors.teal),
          child: AppBar(
            title: const Text('Трекер местоположения',
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          const Expanded(
                              child: Text('История местоположений',
                                  style: TextStyle(
                                      fontSize: 20,
                                      fontWeight: FontWeight.bold,
                                      color: Colors.teal),
                      ...
                      Expanded(
                        child: _locations.isEmpty
                            ? const Center(
                                child: Text('Местоположение пока не сохранено.',
                                    style: TextStyle(
